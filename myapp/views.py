...
def book_appointment(request):
    ...
    existing_appointment = Appointment.objects.filter(
        user=request.user, date__gte=timezone.now().date()
    ).first()

    if existing_appointment:
        messages.error(
            request,
            "You already have a future appointment. You cannot book more than one appointment.",
        )
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            if appointment_minute != 0:
                messages.error(
                    request,
                    "You can only book appointments at the start of the hour (e.g., 1:00, 2:00).",
                )
            ...
            if not (valid_start_hour <= appointment_hour < valid_end_hour):
                messages.error(
                    request, "You can only book appointments between 8 AM and 6 PM."
                )
                return redirect("book_appointment")
...
def appointment_events(request):
    ...
    for appointment in appointments:
        start_lunch = timezone.datetime.combine(
            appointment.date, timezone.datetime.strptime("12:00", "%H:%M").time()
        )
    ...
    for appointment in appointments:
        ...
        writer.writerow(
            [
                ...
                (
                    appointment.photo.url if appointment.photo else ""
                ),  # Photo URL, or empty string if no photo
            ]
        )
...
def mark_appointment_completed(request, appointment_id):
    ...
    messages.success(
        request,
        f"Your appointment on {appointment.date} at {appointment.time} has been marked as completed.",
    )

    # Provide feedback to the owner
    messages.success(
        request,
        f"Appointment for {appointment.user.username} marked as completed and user notified.",
    )
